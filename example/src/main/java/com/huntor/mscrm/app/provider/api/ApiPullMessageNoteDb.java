...
public class ApiPullMessageNoteDb {
     ...
     * @return
     */
    public static List<PullMessageNote> getMsgList(Context context, int dest) {
        ContentResolver resolver = context.getContentResolver();
        Cursor cursor = resolver.query(MSCRMContract.MessageNote.CONTENT_URI, null, MSCRMContract.MessageNote.DEST + "=? ", new String[]{"" + dest}, null);
        List<PullMessageNote> messages = new ArrayList<PullMessageNote>();
        if (cursor != null) {
            PullMessageNote note = null;
            while (cursor.moveToNext()) {
