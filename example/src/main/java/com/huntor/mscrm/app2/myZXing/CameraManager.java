...
package com.huntor.mscrm.app2.myZXing;
...
//      rect.bottom = rect.bottom * cameraResolution.y / screenResolution.y;
   ...
   * 闪光灯开关
   */
  public void flashlightUtils(ImageView mImageView) {
    if (camera == null) {
      camera = Camera.open();
    }
    ...
    if (isFlashlightOn()) {
      ...
      camera.stopPreview();
      camera.startPreview();
      mImageView.setImageResource(R.drawable.light_bacground_off);
      ...
      mImageView.setImageResource(R.drawable.light_bacground_on);
    }
  ...
  public void openLight(){
    try{
      ...
      if(camera.getParameters().getFlashMode().equals(Camera.Parameters.FLASH_MODE_OFF)){
        parameters.setFlashMode(Camera.Parameters.FLASH_MODE_TORCH); // 开始亮灯
  ...
  public void closeLight(){
    try {
      ...
      if(camera.getParameters().getFlashMode().equals(Camera.Parameters.FLASH_MODE_TORCH)) {
        parameters.setFlashMode(Camera.Parameters.FLASH_MODE_OFF);// 关掉亮灯
        //camera.setParameters(parameters);
      }
      camera.setParameters(parameters);
    }catch (Exception e){
